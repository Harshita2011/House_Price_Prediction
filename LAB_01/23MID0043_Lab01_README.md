...
- Models are trained on historical data and may not generalize to future market conditions (inflation, drift).
- California Housing predicts **median block-group value**, not individual house price.
- UCI Real Estate predicts **price per unit area**, not total sale price.
- Location features can encode socioeconomic patterns; use with governance when influencing credit or taxation decisions.
